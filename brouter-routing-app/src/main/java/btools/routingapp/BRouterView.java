...
public class BRouterView extends View {
    ...
    public void pickWaypoints() {
        ...
        if (allpoints == null) {
            ...
            if (allpoints.size() < 2)
                msg = "coordinate source does not contain enough waypoints: " + allpoints.size();
            if (allpoints.size() > 100)
                msg = "coordinate source contains too much waypoints: " + allpoints.size() + "(please use from/to/via names)";
        }
